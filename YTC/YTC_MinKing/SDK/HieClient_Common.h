...
/** @brief 配置信息最大长度											                    			*/
#define MAX_CFG_LEN					(512 * 1024)

/** \brief 配置信息最大长度（V2版本）								                    			*/
#define MAX_CFG_LEN_V2				(128 * 1024 * 2)

/** \brief 探测配置信息最大长度										                    			*/
#define MAX_PARTNER_PROBE_CFG_LEN	(65534)
...
/** @brief 播放控制参数个数																			*/
#define MAX_PLAY_CONTROL_PARAMETER	(6)
...
/** @brief 最大支持分辨率个数																		*/
#define RESOLVE_NUM_MAX				(20)

/** @brief 最大显示设备个数（8个VGA和8个TV）														*/
#define MAX_DISPLAY_DEVICE_NUM		(16)

/** @brief 最大解码模式个数																			*/
#define MAX_DECODE_MODE_NUM		(32)
//////////////////////////////////////////////////////////////////////////
//历史流帧宏定义
//////////////////////////////////////////////////////////////////////////
/** @brief 历史流异常结束																			*/
...
//////////////////////////////////////////////////////////////////////////
/** \brief 远程重启 																			*/
#define HY_DVR_OLD_CONTROL_REBOOT    	120		

/** \brief 高级PTZ云台控制功能 				     												*/
#define HY_DVR_OLD_ADVANCED_PTZ_CONTROL 137  

/** \brief 设备手动录像 				     												*/
#define HY_DVR_OLD_MANUAL_RECORD		136     

/** \brief 获取设备报警IO输入状态 				     												*/
#define HY_DVR_OLD_GET_ALARMINSTATE		133    

/** \brief 获取设备报警IO输出状态 				     												*/
#define HY_DVR_OLD_GET_ALARMOUTSTATE		134   
...
typedef enum eClientSDKAttributeType
{
	...
	eAttributeProbeTimeOut,								/*!< 探测控制超时时间						*/
...
typedef enum eHistoryLogMinorType
{
	...
	eHistoryLogSysOpRemoteBackUp,						/*!< 远程备份								*/
	eHistoryLogSysOpPlayBack,	    					/*!< 回放    								*/
	...
	eHistoryLogNetworkAll			= 700,				/*!< 网络事件所有子项						*/
	eHistoryLogNetworkConnectSuccess,					/*!< 连接成功								*/
	eHistoryLogNetworkConnectFailed,					/*!< 连接失败								*/
	eHistoryLogNetworkDisconnect,						/*!< 网络断开　								*/
	eHistoryLogNetworkRecover,							/*!< 网络恢复								*/
	eHistoryLogNetworkActiveDisconnect,					/*!< 网络主动断开							*/
...
typedef struct tagUserLoginPara
{
	...
	eNetworkSDKVersion	eSDKVersion;					/*!< SDK版本(保留)							*/
	DWORD	dwReserve[DEF_RESERVE_NUM - 1];				/*!< 保留									*/
...
typedef struct tagHYPrivateServerInformation
{
	...
	HistoryStreamMultiTypePara cHistoryStreamPara;				/*!< 历史流参数信息							*/
	DWORD	dwReserve[DEF_RESERVE_NUM];					/*!< 保留									*/
...
/*!
*  @struct	tagHistoryStreamQueryResultEx
*  @brief	历史流查询结果
*/
...
*  @brief	探测用户认证参数
*/
typedef struct tagDeviceProbeUserLoginPara
{
	char	sUName[USERNAME_LEN];						/*!< 登录用户的用户名称						*/
	char	sUPass[USERPASS_LEN];						/*!< 登录用户的用户密码						*/
	DWORD	dwReserve[DEF_RESERVE_NUM];					/*!< 保留									*/
}DeviceProbeUserLoginPara, *LPDeviceProbeUserLoginPara;

/*!
*  @struct	tagDeviceProbeConfigInformation
*  @brief	探测配置信息
*/
typedef struct tagDeviceProbeConfigInformation
{
	BYTE	bytMainCommand;								/*!< 配置信息主命令字						*/
	BYTE	bytAssistCommand;							/*!< 配置信息辅助命令字						*/
	char	sConfig[MAX_PARTNER_PROBE_CFG_LEN];			/*!< 配置信息缓冲区							*/
	WORD	dwConfigLen;								/*!< 配置信息长度							*/
	DWORD	dwReserve[DEF_RESERVE_NUM];					/*!< 保留									*/
}DeviceProbeConfigInformation, *LPDeviceProbeConfigInformation;
...
{
	DWORD	dwDecoderCapacity;								/*!< 解码能力，按位表示，第0位无效，第1位表示“自动识别”，第12位表示“HD1080”（只读配置项）*/
	eResolutionMode	eDecodeMode;							/*!< 当前解码模式								*/
	DWORD	dwDecoderChannelNumber[MAX_DECODE_MODE_NUM];	/*!< 解码通道个数，根据解码模式决定个数，dwDecoderChannelNumber[0]和dwDecoderChannelNumber[13]无效，如果dwDecoderChannelNumber[12]为4，表示解码模式为1080P时，支持4路解码（只读配置项）	*/
	DWORD	dwDisplayDevDecChNum[MAX_DISPLAY_DEVICE_NUM];	/*!< 显示设备对应的解码通道数，数组0表示第一个显示设备，如果dwDisplayDevDecChNum[0]为4，表示该设备有4个解码通道 */
	DWORD	dwReserve[DEF_RESERVE_NUM];						/*!< 保留									*/
...
typedef struct tagDecoderChannelInformation
{
	...
	DWORD	dwConnectState;						/*!< 连接状态：0-连接成功，其它表示错误码			*/
	MediaServerInformation	cMediaServerInfo;			/*!< 流媒体服务器信息							*/
	HYPrivateServerInformation	cHYPrivateServerInfo;	/*!< HY私有协议服务器信息						*/
	LocalHistoryInformation	cLocalHistoryInformation;		/*!< 本地历史流信息							*/
...
typedef struct tagDecoderSetPlayStatusParameter
{
	eStreamPlayControlStatus	ePlayControlStatus;		/*!< 流播放状态控制							*/
		/*	
    1、ePlayControlStatus 为定位时：eStreamPlayControlSetPos
	   元素[0,5]分别表示年、月、日、时、分、秒
	*/
...
typedef struct tagDecoderPlayStatusParameter
{
	...
	eStreamPlayControlStatus	ePlayControlStatus;		/*!< 流播放状态控制							*/
	/*	
	1、ePlayControlStatus 为速度调节时: eStreamPlayControlFast, eStreamPlayControlSlow.
	元素dwParameter[0]表示速度, 1表示1/16倍速, 2表示1/8倍速, 3表示1/4倍速, 4表示1/2倍速，5表示1倍速，6表示2倍速，7表示4倍速，8表示8倍速，9表示16倍速；
	*/	
	DWORD dwParameter[MAX_PLAY_CONTROL_PARAMETER];		/*!< 流播放状态控制参数						*/
	DWORD		dwEnableEndTime;						/*!< 结束时间是否有效						*/
	TimeInfo	cBeginTime;								/*!< 历史流起始时间							*/

...
#define  MEDIA_TYPE_PCMA_HISI	(BYTE)22//G.711a-hisi
// #endif//MEDIA_TYPE
...
struct NETDVR_VOIPProperty_t 
{
	...
	unsigned char   VOIPMediaType;					//语音音频类型
...
enum NETDVR_PROTOCOLTYPE {
	...
	NETDVR_PROTOCOLTYPE_SOUNY = 4,		//protocol type : Souny
};
...
struct NETDVR_ptzParam_t 
{
	...
	unsigned char autoflag;							//自动检测日夜模式
...
enum NETDVR_PTZCONTROL {
	...
	NETDVR_PTZ_COM_IRISLARGESTOP = 25,
	NETDVR_PTZ_COM_IRISSMALLSTOP = 26,
};
...
typedef enum NETDVR_BITRATE {
	...
	NETDVR_BITRATE_3072 = 3072,				//3Mbps
	NETDVR_BITRATE_4096 = 4096,				//4Mbps
	NETDVR_BITRATE_5120 = 5120,				//5Mbps
	NETDVR_BITRATE_6144 = 6144,				//6Mbps
	NETDVR_BITRATE_7168 = 7168,				//7Mbps
	NETDVR_BITRATE_8192 = 8192,				//8Mbps
	NETDVR_BITRATE_9216 = 9216,				//9Mbps
	NETDVR_BITRATE_10240 = 10240,			//10Mbps
} bitrate_t;
...
struct NETDVR_alarmInParam_t
{
	...
	unsigned char		flag_buzz;					//触发蜂鸣器，IPC是用来作截图上传方式标志
	...
	unsigned char		flag_ptz;					//触发PTZ联动
...
struct NETDVR_VideoLostParam_t
{
	...
	unsigned int 		trigRecChn;					//触发录像通道 按位
	unsigned int 		trigAlarmOut;				//触发报警输出 按位
	unsigned char 		flag_buzz;					//蜂鸣器, IPC是用来作截图上传方式标志
	unsigned char		flag_email;					//触发emaill
	unsigned char		flag_mobile;				//触发手机报警
	delay_t 			delay;						//延时
	unsigned int 		copy2Chnmask;				//复制到其他通道。按位
...
struct NETDVR_VideoBlockParam_t
{
	...
	unsigned int 		trigRecChn;					//触发录像通道 按位
	unsigned int 		trigAlarmOut;				//触发报警输出 按位
	unsigned char 		flag_buzz;					//蜂鸣器, IPC是网络不通
	unsigned char		flag_email;					//触发emaill
	unsigned char		flag_mobile;				//触发手机报警
	delay_t				delay;						//延时
	unsigned int 		copy2Chnmask;				//复制到其他通道。按位
...
//网络不通
struct NETDVR_NoNetworkParam_t
{
	unsigned char		chn;
	unsigned int 		trigRecChn;					//触发录像通道 按位
	unsigned int 		trigAlarmOut;				//触发报警输出 按位
...
#define NETDVR_VIDEORESOLU_720P     5 //720P
#define NETDVR_VIDEORESOLU_1080P    6 //1080P
#define NETDVR_VIDEORESOLU_END		NETDVR_VIDEORESOLU_1080P //在NETDVR_VIDEORESOLU_1080P这里结束，可以通过判断是否在BEGIN和END之间来确认是否正确
...
// wifi数据结构
struct NETDVR_WIFIinfo
{
    char			name[64];       // 网络名
    char			passwd[64];     // 密码
    char			quality[10];    // 信号强度
	unsigned char	connectflag;	// 连接标志
	unsigned int	wifiip;
    unsigned char	reserved[17];
};

struct NETDVR_WIFINumber_t 
{
	struct NETDVR_WIFIinfo wifiinfo[8];
};

struct NETDVR_PortInfo
{
	unsigned short orderport;
	unsigned short upleftport;
	unsigned short uprightport;
	unsigned short downleftport;
	unsigned short downrightport;
	unsigned short highresloport;
	unsigned char  reserved[4];
};

struct NETDVR_HDMIInfo
{
	unsigned short vgasol;
	unsigned short refreshrate;
	unsigned char  standard; //1=NTSC; 2=PAL
...
int __stdcall NETDVR_VOIPRegRcvCB(int Handle, int voipindex, pFrameCallBack rcvfunc, unsigned int dwContext);
int __stdcall NETDVR_VOIPRegCaptureCB(int Handle, int voipindex, pFrameCallBack getcapturefunc, unsigned int dwContext);
int __stdcall NETDVR_VOIPSetSendMode(int Handle, int voipindex, unsigned char flagmode);
int __stdcall NETDVR_VOIPSendData(int Handle, int voipindex, FrameHeadr voipdata);
...
//透明串口
typedef void(CALLBACK*  pSerialDataCallBack)(int lSerialPort, unsigned char bySerialChannel, unsigned char *pRecvDataBuffer, unsigned int dwBufSize, unsigned int dwContent);
...
int __stdcall NETDVR_SetVideoProperty(int Handle, const struct NETDVR_VideoProperty_t *p_para);
